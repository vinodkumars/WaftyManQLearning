...
(function (Ω) {
	...
	input = {
		...
		init: function (dom, icade) {
			...
			icade = false;//icade || Ω.urlParams.icade;
...
(function (Ω) {
	...
	var Physics = {
		...
		checkPointCollision: function (point, entity, entities, cbName) {
			...
			for (i = 0; i < len; i++) {
				...
				if (ax >= bx &&
					...
					ay >= b.y &&
					ay <= b.y + b.h) {
					entity[cbName] && entity[cbName](b);
					b[cbName] && b[cbName](entity);
					return true;
				}
			}
...
(function (Ω) {
	...
	var SpriteAtlas = Ω.Class.extend({
		...
		render_stretch: function(gfx, name, x, y) {
			var img = this.images.main,
				imgData = this.csv[name];
...
(function (Ω) {
	...
	var Game = Ω.Class.extend({
		...
		run: function () {
			...
			var self = this,
				now = Date.now(),
				frameTime = Math.min((now - this.currentTime) / 1000, this.preset_dt) * heartbeatMultiplier,  // EDIT: added multiplier
				c;
	...
	*/
	function initCanvas(canvasSelector, w, h) {
		...
		var selCanvas = document.getElementById(canvasSelector),
			newCanvas,
			ctx;
		...
		if (selCanvas.nodeName.toUpperCase() === "CANVAS") {
			var explicitWidth = selCanvas.width,
				explicitHeight = selCanvas.height;
